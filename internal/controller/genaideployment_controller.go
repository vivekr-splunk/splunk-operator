...
func (r *GenAIDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	// your logic here
	reconcileCounters.With(getPrometheusLabels(req, "GenAIDeployment")).Inc()
	defer recordInstrumentionData(time.Now(), req, "controller", "GenAIDeployment")

	reqLogger := log.FromContext(ctx)
	reqLogger = reqLogger.WithValues("GenAIDeployment", req.NamespacedName)

	// Fetch the ClusterManager
	instance := &enterpriseApi.GenAIDeployment{}
	...
	if annotations != nil {
		if _, ok := annotations[enterpriseApi.GenAIDeploymentPausedAnnotation]; ok {
