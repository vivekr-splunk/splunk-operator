...
import (
	...
	enterprisev3 "github.com/vivekrsplunk/splunk-operator/api/v3"
	enterprisev4 "github.com/vivekrsplunk/splunk-operator/api/v4"
	"github.com/vivekrsplunk/splunk-operator/internal/controller"
	//+kubebuilder:scaffold:imports
	externalrayio "github.com/ray-project/kuberay/ray-operator/apis/ray/v1"
)
...
func init() {
	...
	utilruntime.Must(externalrayio.AddToScheme(scheme))
	utilruntime.Must(externalrayio.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "05513e59.splunk.com",
	...
	if err = (&controller.ClusterMasterReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.LicenseMasterReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err = (&controller.GenAIDeploymentReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("genaideployment-controller"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "GenAIDeployment")
		os.Exit(1)
	}
